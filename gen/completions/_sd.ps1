...
Register-ArgumentCompleter -Native -CommandName 'sd' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'sd' {
            ...
            [CompletionResult]::new('-L', 'L ', [CompletionResultType]::ParameterName, 'Process input line by line instead of reading the entire input at once. This reduces memory usage and enables streaming for stdin, but prevents patterns from matching across line boundaries')
            [CompletionResult]::new('--line-by-line', 'line-by-line', [CompletionResultType]::ParameterName, 'Process input line by line instead of reading the entire input at once. This reduces memory usage and enables streaming for stdin, but prevents patterns from matching across line boundaries')
